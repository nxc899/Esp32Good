...
uint8_t touch_scan(void)
{
    ...
    i2c_master_transmit_receive(dev_handle,wbuf,1,rbuf,1,10);
...
void get_touch_pos(void)
{
    ...
    i2c_master_transmit_receive(dev_handle,wbuf,1,rbuf,4,10);

    touch_dev.x = (uint16_t)(rbuf[0]&0x0F<<8) + rbuf[1];
    touch_dev.y = (uint16_t)(rbuf[2]&0x0F<<8) + rbuf[3];
}
...
void touch_init(void)
{
    ...
    *wbuf = FT_ID_G_THGROUP;
    i2c_master_transmit(dev_handle,wbuf,1,10);
}
